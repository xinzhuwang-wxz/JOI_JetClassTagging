...
class MIParticleTransformerWrapper(torch.nn.Module):
    def __init__(self, **kwargs) -> None:
        ...
        fc_params = kwargs.pop('fc_params')  # pop means remove the key from the dictionary and return the value
        num_classes = kwargs.pop('num_classes')
        self.for_inference = kwargs['for_inference']

        fcs = []  # list of fully connected layers
        for out_dim, drop_rate in fc_params:
            fcs.append(nn.Sequential(nn.Linear(in_dim, out_dim), nn.ReLU(), nn.Dropout(drop_rate)))
            in_dim = out_dim
        fcs.append(nn.Linear(in_dim, num_classes))
...
def get_model(data_config, **kwargs):
    ...
    cfg = dict(
        ...
        embed_dims=[128, 512, 128],  # to match D1 = 128, it is MIParT-L, for MIParT, it is [128,512,64]
        pair_embed_dims=[64, 64, 64],
        num_heads=8,
        num_MIlayers=5,
        num_layers=5,
