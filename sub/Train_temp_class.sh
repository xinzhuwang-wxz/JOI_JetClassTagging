...
extraopts=""
if [[ "$model" == "ParT" ]]; then
    modelopts="${_DIR}/networks/example_ParticleTransformer.py --use-amp"
#    lr="1e-3"
elif [[ "$model" == "ParT-FineTune" ]]; then
    modelopts="${_DIR}/networks/example_ParticleTransformer_finetune.py --use-amp"
#    lr="1e-4"
...
$CMD \
    ...
    --model-prefix pt/net \
    $dataopts \
    --num-workers 8 --fetch-step 0.01 --batch-size BATCHSIZE \
    --samples-per-epoch SAMPLEperEPOCH --samples-per-epoch-val SAMPLEPerEPOCHval --num-epochs EPOCHS --gpus '0' \
    --start-lr LR --optimizer ranger --log logs/Log.log --predict-output output/pred.root \
    --tensorboard TB \
    ${extraopts}

 # no in-memory except qg
